...
// @match        https://nt.site777.tv/data/*
// @match        https://nt.site777.tv/data/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=site777.tv
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  if (errorExist) {
    if (
      errorExist.innerHTML.indexOf("現在システムが混み合っております。") != -1
    ) {
  ...
  if (
    ...
    HallSelectLink();
  } else if (location.href.indexOf("/MypageLogin.do") != -1) {
    MypageLoginOrTop();
  ...
  if (title.textContent.indexOf("設置機種&出玉情報") != -1) {
    ...
    for (const key in tds) {
      ...
      if (typeof element == "object") {
        var name = textTrim(element.querySelector("span").textContent).split(
          "（"
        )[0];
        console.log(name);
  ...
  function ichiranOrGraphs() {
    ...
    if (ooatari) {
      ...
      if (a) {
        makeBtnOnClick("graph", document.querySelector("#g-nav"), a);
      } else {
      ...
      for (let i = 0; i < days.length; i++) {
        ...
        for (let j = 0; j < element.length; j++) {
          ...
          var aboutCoin = Math.round(
            jug.samaiAbout(o["BB"], o["RB"], o["trials"], result.settei - 1)
          );
          sum += aboutCoin;
        }
        daySum.push(sum);
    ...
    if (graph_list) {
      ...
      //https://nt.site777.tv/data/WUl6dEJoc1FoK0g0akJobEluK1IrNGwrdzdRUXNQNWNPVlFRTlcrWmRWVTlYMG9yenpiRmxUdURHY0czYlovQkNyYVFKUzBLM0E2RGlwdDRyN3lnekE9PQ==/kakin/GraphList.do?hallcode=2c35376356c331d3ab511e3c84471807&tablenum=2fa360848a39be0b&tablelistflag=1&day=0&currentpageno=1&uritanka=2173&modelcode=119916'
      var day = getDayNo();
      autoSave(graphData, machine_name + "_graph_" + "day" + day + ".txt");
  ...
  function daySelectAddDate(daySum) {
    ...
    var fnc = function (o, before = 0) {
      ...
      return {
        tex:
          o.getMonth() + 1 + "/" + o.getDate() + "(" + dlist[o.getDay()] + ")",
        color: colors[o.getDay()],
        fontColor: getContrastYIQ(colors[o.getDay()].split("#").join("")),
      };
    };
    // ;
    for (let i = 0; i < dates.length; i++) {
      ...
      div.innerHTML = o.tex + "<br>" + daySum[i] + "枚";
      div.setAttribute(
        "style",
  ...
  function myPageLoginTop() {
    ...
    mail.value = "msnryng@gmail.com";
    var pass = document.querySelector("#mypage_pass");
    pass.value = "anthrax871";
  ...
  function autoSave(tex, filename) {
    var hallname = document.querySelectorAll("meta")[0].content.split(",")[0];
    var a = document.createElement("a");
    ...
    var fnc = function () {
      ...
      for (let index = 0; index < data.length; index++) {
        ...
        if (result.settei > 3 && result.ruiseki <= 0.1) {
          var rstarNum = Math.round((1 - result.ruiseki / 0.1) * 5);
        ...
        table.setAttribute(
          "style",
          "width:200px!important;height:" + clientRect.height + "px!important;"
        );
        ...
        tr.appendChild(td);
        td.textContent =
          "設定" + result.settei + " (" + (result.p * 100).toFixed(2) + "%)";
        td.setAttribute(
          "style",
          "text-align: left;margin-left: 0;width:90px!important;"
        );

        var td = document.createElement("td");
        tr.appendChild(td);
        td.textContent = starTex;
        td.setAttribute("style", "text-align: left;margin-left: 0;");

        var td = document.createElement("td");
        tr.appendChild(td);
        td.textContent = aboutCoin + "枚";
        td.setAttribute("style", "text-align: right;width:60px!important;");
        ...
        tr.appendChild(td);
        td.innerHTML =
          '<span style="font-size: 11px !important;margin-left: 10px;">S1検定:' +
  ...
  function textTrim(tex, option) {
    return tex.split("\t").join("").split("\n").join("").split(option).join("");
  }
  ...
  class JugData {
    constructor() {
      ...
      this.SEBONUS["SアイムジャグラーＥＸ"] = [
        { SB: 431.157895, EB: 744.727273, SR: 642.509804, ER: 1394.382979 },
        { SB: 422.812903, EB: 744.727273, SR: 590.414414, ER: 1236.528302 },
        { SB: 422.812903, EB: 744.727273, SR: 474.898551, ER: 1092.266667 },
        { SB: 417.426752, EB: 682.666667, SR: 448.876712, ER: 1057.032258 },
        { SB: 417.426752, EB: 682.666667, SR: 364.088889, ER: 851.116883 },
        { SB: 407.055901, EB: 682.666667, SR: 364.088889, ER: 851.116883 },
      ];
      this.SEBONUS["マイジャグラーV"] = [
        { SB: 409.6, EB: 819.2, SR: 655.36, ER: 1092.266667 },
        { SB: 407.055901, EB: 809.08642, SR: 601.247706, ER: 1074.360656 },
        { SB: 399.609756, EB: 799.219512, SR: 492.75188, ER: 1057.032258 },
        { SB: 378.820809, EB: 771.011765, SR: 407.055901, ER: 1008.246154 },
        { SB: 354.248649, EB: 744.727273, SR: 390.095238, ER: 862.315789 },
        { SB: 337.814433, EB: 712.347826, SR: 327.68, ER: 762.046512 },
      ];
      this.SEBONUS["ファンキージャグラー２ＫＴ"] = [
        { SB: 402.06135, EB: 789.590361, SR: 636.271845, ER: 1424.695652 },
        { SB: 397.187879, EB: 744.727273, SR: 574.877193, ER: 1394.382979 },
        { SB: 397.187879, EB: 720.175824, SR: 512.0, ER: 1285.019608 },
        { SB: 385.505882, EB: 704.688172, SR: 448.876712, ER: 1149.754386 },
        { SB: 378.820809, EB: 655.36, SR: 409.6, ER: 1110.779661 },
        { SB: 339.564767, EB: 624.152381, SR: 356.173913, ER: 992.969697 },
      ];
    ...
    samaiAbout(BB, RB, n, settei) {
      ...
      var coinIn = n * 3;
      var coinOut = BB * this.bbCoin + RB * this.rbCoin + budou + cherry + rep;
    ...
    suisoku(BB, RB, n) {
      ...
      var ruiseki = this.binom.compute(RB, n, 1 / this.machine[0][3]);
      return { settei: result[0].settei, p: result[0].val, ruiseki: ruiseki };
    }
  }
  //-----------------------------------------------------------------------
  ...
  class Binom {
    ...
    BinomTerm(k, n, p) {
      return Math.exp(
        this.LnComb(n, k) + k * Math.log(p) + (n - k) * Math.log(1 - p)
      );
    }
    ...
    LnFact(x) {
      x = x <= 1 ? 1 : x;
      return x < 12 ? Math.log(this.Fact(Math.round(x))) : this.LnFactSub(x);
    }
    ...
    compute(X, N, P, sw) {
      ...
      if (!sw && X / N >= P) {
        bincdf = 1 - this.compute(X != 0 ? X - 1 : X, N, P, true);
      }
      return bincdf;
    }
    ...
    normalcdf(z) {
      ...
      if (37 >= a) {
        ...
        b =
          7.07106781186547 > a
            ? (b *
                ((((((0.0352624965998911 * a + 0.700383064443688) * a +
                  6.37396220353165) *
                  a +
                  33.912866078383) *
                  a +
                  ...
                  a +
                  220.206867912376)) /
              (((((((0.0883883476483184 * a + 1.75566716318264) * a +
                16.064177579207) *
                a +
                86.7807322029461) *
                a +
